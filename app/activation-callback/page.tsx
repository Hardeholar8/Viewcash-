  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (!txRef && !transactionId) {
        setMessage("No payment reference was returned. If you were charged, please contact support.");
        return;
      }
      for (let attempt = 0; attempt < 4; attempt++) {
        try {
          ...
          setMessage(data.error || "Payment was not completed. Your account was not activated.");
          return;
        } catch {
          if (attempt < 3) await new Promise((resolve) => setTimeout(resolve, 2000));
        }
      }
      setMessage("We could not confirm the payment yet. Your account has not been activated. Please try again shortly.");
    };
    run();
  }, []);
  ...
  return (
    <main className="mx-auto min-h-screen max-w-md bg-[#0b1020] px-5 py-20 text-white">
      <div className="rounded-3xl border border-white/10 bg-white/[.04] p-7 text-center">
        ...
        <h1 className="mt-5 text-2xl font-bold">{ok ? "Account Activated" : "Payment Status"}</h1>
