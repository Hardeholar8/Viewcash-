...
function validateInitData(initData: string, botToken: string) {
  ...
  // Telegram Mini App validation: HMAC-SHA256 with the bot token as the key
  // and "WebAppData" as the message.
  ...
  try {
    ...
    if (!botToken || !url || !serviceRoleKey) {
      return NextResponse.json(
        { error: "ViewCash server is not fully configured" },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json(
      { error: "Unable to connect ViewCash account" },
      { status: 500 }
    );
  }
}
