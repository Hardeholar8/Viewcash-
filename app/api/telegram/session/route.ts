...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: existing, error: lookupError } = await supabase
      ...
      .maybeSingle();
    if (lookupError) throw new Error(`SUPABASE_USER_LOOKUP_ERROR:${lookupError.code || "UNKNOWN"}`);
    ...
    if (!user) {
      ...
      if (error) throw new Error(`SUPABASE_USER_CREATE_ERROR:${error.code || "UNKNOWN"}`);
      user = data;
      const { error: walletError } = await supabase.from("wallets").insert({ user_id: user.id });
      if (walletError) throw new Error(`SUPABASE_WALLET_CREATE_ERROR:${walletError.code || "UNKNOWN"}`);
    } else {
      ...
      if (error) throw new Error(`SUPABASE_USER_UPDATE_ERROR:${error.code || "UNKNOWN"}`);
      user = data;
    }
    ...
    if (walletError) throw new Error(`SUPABASE_WALLET_LOOKUP_ERROR:${walletError.code || "UNKNOWN"}`);
