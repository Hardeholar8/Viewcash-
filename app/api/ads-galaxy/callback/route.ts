...
export async function POST(req: Request) {
  try {
    ...
    if (!timestamp || !headerEventId || !supplied || Math.abs(Date.now() / 1000 - Number(timestamp)) > 300) return NextResponse.json({ error: "INVALID_SIGNATURE" }, { status: 401 });
    const expected = crypto.createHmac("sha256", CALLBACK_SECRET).update(timestamp + "." + headerEventId + "." + rawBody).digest("hex");
    if (supplied.length !== expected.length || !crypto.timingSafeEqual(Buffer.from(supplied), Buffer.from(expected))) return NextResponse.json({ error: "INVALID_SIGNATURE" }, { status: 401 });
    const payload = JSON.parse(rawBody);
    if (payload?.event_id !== headerEventId || payload?.status !== "completed" || String(payload?.mini_app_id) !== "92") return NextResponse.json({ error: "INVALID_EVENT" }, { status: 400 });
    const telegramId = String(payload?.user_id || "").trim();
    const eventId = String(payload?.event_id || "").trim();
    const requestId = String(payload?.request_id || "").trim();
    if (!telegramId || !eventId || !requestId) return NextResponse.json({ error: "INVALID_PAYLOAD" }, { status: 400 });
    ...
    const { data, error } = await db.rpc("credit_adsgalaxy_reward", { p_user_id: user.id, p_event_id: eventId, p_request_id: requestId, p_reward_coins: rewardCoins });
