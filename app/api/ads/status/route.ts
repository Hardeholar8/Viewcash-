...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: session, error: sessionError } = await db
      ...
      .eq("ymid", ymid)
      .maybeSingle();
    if (sessionError) return NextResponse.json({ error: "DATABASE_ERROR" }, { status: 500 });
    if (!session) return NextResponse.json({ confirmed: false, status: "not_found" });

    const { data: wallet, error: walletError } = await db.from("wallets").select("coins").eq("user_id", user.id).maybeSingle();
    if (walletError) return NextResponse.json({ error: "DATABASE_ERROR" }, { status: 500 });

    return NextResponse.json({
      confirmed: session.status === "completed",
      status: session.status,
      coins: Number(wallet?.coins || 0),
    });
  } catch (error) {
    console.error("ViewCash ad status error", error);
    return NextResponse.json({ error: "AD_STATUS_ERROR" }, { status: 500 });
  }
}
