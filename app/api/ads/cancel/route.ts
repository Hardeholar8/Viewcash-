...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!session) return NextResponse.json({ cancelled: false, status: "not_found" });
    if (session.status === "completed") return NextResponse.json({ cancelled: false, status: "completed" });
    ...
    console.error("ViewCash ad cancel error", error);
    return NextResponse.json({ error: "AD_CANCEL_ERROR" }, { status: 500 });
  }
}
