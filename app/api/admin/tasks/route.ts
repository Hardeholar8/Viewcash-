...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabase.from("tasks").select("id,title,description,reward,task_type,action_url,daily_limit,completion_limit,completed_count,status,proof_required,verification_type,telegram_chat,created_at,updated_at").order("created_at", { ascending: false });
