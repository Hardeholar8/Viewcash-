...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!initData || !botToken || !serviceKey) return NextResponse.json({ error: "ADMIN_CONFIG_ERROR" }, { status: 500 });
    ...
    const supabase = createClient(VIEWCASH_SUPABASE_URL, serviceKey, { auth: { autoRefreshToken: false, persistSession: false } });
    ...
    return NextResponse.json({ error: "ADMIN_SESSION_ERROR" }, { status: 500 });
  }
}
