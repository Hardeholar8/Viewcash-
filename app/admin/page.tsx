...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/users${search ? `?search=${encodeURIComponent(search)}` : ""}`, { cache: "no-store" })
  ...
  return <main className="min-h-screen bg-[#080d18] text-white md:flex">
    ...
    <section className="mx-auto w-full max-w-6xl p-5 md:p-8"><header className="mb-7"><p className="text-sm text-slate-500">ViewCash control center</p><h2 className="mt-1 text-3xl font-extrabold">{active}</h2></header>
      ...
      {active !== "Dashboard" && active !== "Users" && <div className="rounded-3xl border border-white/10 bg-white/[.035] p-6"><h3 className="text-xl font-bold">{active}</h3><p className="mt-2 text-sm text-slate-400">This section is ready for its secure database controls.</p></div>}
    </section>
  </main>;
}
