...
export default function HomePage() {
  ...
  const refresh = async (tg: TelegramWebApp) => {
    const data = tg.initData?.trim();
    if (!data) throw new Error("Telegram session data is unavailable. Please reopen ViewCash from Telegram.");
  ...
  const loadTasks = async () => {
    ...
    try {
      ...
      setTasks(d.tasks || []);
    } catch (e) { setMessage(e instanceof Error ? e.message : "Unable to load tasks"); }
  ...
  const verify = async (task: Task) => {
    ...
    try {
      const r = await fetch("/api/tasks", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ task_id: task.id, initData }), cache: "no-store" });
      const d = await r.json().catch(() => ({}));
  ...
  const withdraw = async () => {
    const amount = Number(withdrawCoins); const available = withdrawWallet === "affiliate" ? referralCoins : coins;
    ...
    setBusy("withdraw");
    try {
      const r = await fetch("/api/withdrawals", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ initData, bank_name: bankName, account_name: accountName, account_number: accountNumber, coins: amount, wallet_type: withdrawWallet }), cache: "no-store" });
      const d = await r.json().catch(() => ({}));
      ...
      setWithdrawCoins(""); setMessage(`${withdrawWallet === "affiliate" ? "Affiliate" : "Task"} wallet withdrawal submitted. Cash value: ₦${Number(d.cash_amount || 0).toFixed(2)}`);
